...
contract CPMMFuzzTest is Test {
    ...
    function setUp() public {
        ...
        stakeToken.mint(creator, 50000e18);
        stakeToken.mint(trader1, 50000e18);
        stakeToken.mint(trader2, 50000e18);
        stakeToken.mint(trader3, 50000e18);
    ...
    function testFuzz_ConstantProductInvariant(
        ...
        tradeAmount = bound(tradeAmount, 1e15, 10000e18); // 0.001 to 10,000 tokens
     ...
     */
    function testFuzz_SharesVsReservesInvariant(
        uint256[3] memory tradeAmounts,
     ...
     */
    function skip_testFuzz_NoImmediateArbitrage(
        uint256 tradeAmount,
        uint8 outcome
    ) public {
     ...
     */
    function testFuzz_FeeAccumulation(
        uint256[5] memory tradeAmounts,
        uint8[5] memory outcomes
    ) public {
        CPMMMarketImplementation cpmmMarket = CPMMMarketImplementation(market);
     ...
     */
    function testFuzz_MarketSolvency(
        uint256[4] memory tradeAmounts,
        uint8[4] memory outcomes
    ) public {
        CPMMMarketImplementation cpmmMarket = CPMMMarketImplementation(market);
        ...
        for (uint256 i = 0; i < 4; i++) {
            ...
            try cpmmMarket.buyShares(outcomes[i], tradeAmounts[i], 0) {
                ...
                uint256 maxPayout = cpmmMarket.sharesYES(trader1) + 
                                  cpmmMarket.sharesYES(trader2) + 
                                  cpmmMarket.sharesYES(trader3) + 
                                  cpmmMarket.sharesYES(creator);
                ...
                uint256 maxPayoutNO = cpmmMarket.sharesNO(trader1) + 
                                    cpmmMarket.sharesNO(trader2) + 
                                    cpmmMarket.sharesNO(trader3) + 
                                    cpmmMarket.sharesNO(creator);
